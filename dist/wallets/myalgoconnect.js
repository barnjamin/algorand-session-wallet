var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        var v = factory(require, exports);
        if (v !== undefined) module.exports = v;
    }
    else if (typeof define === "function" && define.amd) {
        define(["require", "exports", "algosdk", "@randlabs/myalgo-connect"], factory);
    }
})(function (require, exports) {
    "use strict";
    Object.defineProperty(exports, "__esModule", { value: true });
    var algosdk_1 = __importDefault(require("algosdk"));
    var myalgo_connect_1 = __importDefault(require("@randlabs/myalgo-connect"));
    var logoInverted = "data:image/png;base64,";
    var logo = "data:image/png;base64,";
    /*
     Warning:
        Popups will be blocked if the connect or sign are called outside of a user triggered event, please make sure
        the user allows all popups or the calls are all triggered by events
        https://connect.myalgo.com/docs/getting-started/important-considerations#browser-events-and-blocked-popups
    */
    var MyAlgoConnectWallet = /** @class */ (function () {
        function MyAlgoConnectWallet() {
            this.accounts = [];
            this.defaultAccount = 0;
            this.walletConn = new myalgo_connect_1.default();
        }
        MyAlgoConnectWallet.displayName = function () {
            return "My Algo";
        };
        MyAlgoConnectWallet.prototype.displayName = function () {
            return MyAlgoConnectWallet.displayName();
        };
        MyAlgoConnectWallet.img = function (inverted) {
            return inverted ? logoInverted : logo;
        };
        MyAlgoConnectWallet.prototype.img = function (inverted) {
            return MyAlgoConnectWallet.img(inverted);
        };
        MyAlgoConnectWallet.prototype.connect = function () {
            return __awaiter(this, void 0, void 0, function () {
                var accounts, err_1;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            if (this.isConnected())
                                return [2 /*return*/, true];
                            _a.label = 1;
                        case 1:
                            _a.trys.push([1, 3, , 4]);
                            return [4 /*yield*/, this.walletConn.connect()];
                        case 2:
                            accounts = _a.sent();
                            this.accounts = accounts.map(function (account) { return account.address; });
                            return [3 /*break*/, 4];
                        case 3:
                            err_1 = _a.sent();
                            return [2 /*return*/, false];
                        case 4: return [2 /*return*/, true];
                    }
                });
            });
        };
        MyAlgoConnectWallet.prototype.isConnected = function () {
            return this.accounts && this.accounts.length > 0;
        };
        MyAlgoConnectWallet.prototype.disconnect = function () {
            /* noop */
        };
        MyAlgoConnectWallet.prototype.getDefaultAccount = function () {
            if (!this.isConnected())
                return "";
            return this.accounts[this.defaultAccount];
        };
        MyAlgoConnectWallet.prototype.doSign = function (defaultAcct, txns) {
            return __awaiter(this, void 0, void 0, function () {
                var unsigned, signedTxns, tidx, txn, s, x;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            unsigned = [];
                            signedTxns = [];
                            for (tidx in txns) {
                                if (!txns[tidx])
                                    continue;
                                txn = txns[tidx];
                                if (algosdk_1.default.encodeAddress(txn.from.publicKey) === defaultAcct) {
                                    signedTxns.push(unsigned.length);
                                    unsigned.push(txn.toByte());
                                }
                                else {
                                    signedTxns.push({ txID: "", blob: new Uint8Array() });
                                }
                            }
                            return [4 /*yield*/, this.walletConn.signTransaction(unsigned)];
                        case 1:
                            s = _a.sent();
                            for (x = 0; x < signedTxns.length; x++) {
                                if (typeof signedTxns[x] === "number")
                                    signedTxns[x] = s[signedTxns[x]];
                            }
                            return [2 /*return*/, signedTxns];
                    }
                });
            });
        };
        MyAlgoConnectWallet.prototype.signTxn = function (txns) {
            return __awaiter(this, void 0, void 0, function () {
                var defaultAcct;
                var _this = this;
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            defaultAcct = this.getDefaultAccount();
                            if (!this.permissionCallback) return [3 /*break*/, 2];
                            return [4 /*yield*/, this.permissionCallback.request({
                                    approved: function () { return __awaiter(_this, void 0, void 0, function () {
                                        return __generator(this, function (_a) {
                                            switch (_a.label) {
                                                case 0: return [4 /*yield*/, this.doSign(defaultAcct, txns)];
                                                case 1: return [2 /*return*/, _a.sent()];
                                            }
                                        });
                                    }); },
                                    declined: function () { return __awaiter(_this, void 0, void 0, function () {
                                        return __generator(this, function (_a) {
                                            return [2 /*return*/, []];
                                        });
                                    }); },
                                })];
                        case 1: return [2 /*return*/, _a.sent()];
                        case 2: return [4 /*yield*/, this.doSign(defaultAcct, txns)];
                        case 3: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        };
        MyAlgoConnectWallet.prototype.signBytes = function (b, permissionCallback) {
            throw new Error("Method not implemented.");
        };
        MyAlgoConnectWallet.prototype.signTeal = function (teal, permissionCallback) {
            return __awaiter(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, this.walletConn.signLogicSig(teal, this.getDefaultAccount())];
                        case 1: return [2 /*return*/, _a.sent()];
                    }
                });
            });
        };
        return MyAlgoConnectWallet;
    }());
    exports.default = MyAlgoConnectWallet;
});
