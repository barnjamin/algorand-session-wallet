...
class AlgoSignerWallet {
    ...
    signTxn(txns) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const encoded_txns = txns.map((tx) => {
                const t = { txn: AlgoSigner.encoding.msgpackToBase64(tx.toByte()) };
                //@ts-ignore
                if (algosdk_1.default.encodeAddress(tx.from.publicKey) !== default_acct)
                    t.signers = [];
                return t;
            });
            const signed = yield AlgoSigner.signTxn(encoded_txns);
